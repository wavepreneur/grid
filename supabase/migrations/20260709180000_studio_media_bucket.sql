...
drop policy if exists "studio_media_service_upload" on storage.objects;
create policy "studio_media_service_upload"
  on storage.objects for insert
  with check (bucket_id = 'studio-media');

drop policy if exists "studio_media_service_update" on storage.objects;
create policy "studio_media_service_update"
  on storage.objects for update
  using (bucket_id = 'studio-media');

drop policy if exists "studio_media_service_delete" on storage.objects;
